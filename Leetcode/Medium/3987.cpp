...
Output: 0
Explanation:
To process al elements, we can use the initial 10 units of resources without performing any operations. Thus, the total cost required is 0.*/

//*  ***__int128 use kiya hai ***
class Solution
{
public:
    long long mod = 1e9 + 7;
    ...
    {
        int n = a.size();
        long long cost = 0, last = 1, curr = k;
        for (int i = 0; i < n; i++)
        {
            if (curr < a[i])
            {
                long long diff = a[i] - curr;
                long long t = (diff + k - 1) / k;
                __int128 sum = (__int128)t * (2LL * last + t - 1);
                sum /= 2;
