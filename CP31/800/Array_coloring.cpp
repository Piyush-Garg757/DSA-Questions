...
Your task is to determine whether it is possible to color all its elements in two colors in such a way that:
- the sums of the elements of both colors have the same parity, and
...
Output
For each test case, output "YES" (without quotes) if it is possible to color the array in two colors in such a way that:
- the sums of the elements of both colors have the same parity, and
- each color has at least one element colored,
and "NO" otherwise.
...
In the second sample, there are only two colorings:
[4,7]
and
[4,7]
but in both cases the parity of sums is different.
In the third sample, you can color:
[3,9,8]
and 12 and 8 are both even.*/
